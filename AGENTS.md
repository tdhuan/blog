...
- Hairline separation via `border-t` / `divide-y divide-outline-variant` (see Footer, Header's mobile menu); vertical hairlines via `divide-x` / `border-l` for side-by-side panels (see About's "Things I enjoy" / "Outside of code" row).
